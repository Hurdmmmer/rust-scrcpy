...
3. 复制 DLL 到 Flutter thirdparty 目录
...
try {
    ...
    $DstDll = Join-Path $FlutterRoot "lib\platforms\windows\thirdparty\rust_scrcpy.dll"
    ...
    Invoke-Step "FRB 代码生成" {
        ...
        flutter_rust_bridge_codegen generate `
          --rust-root $RustRoot `
          --rust-input crate::rust_scrcpy_api `
    ...
    Invoke-Step "写入 Dart 兼容入口（rust_scrcpy_api.dart）" {
        $CompatFile = Join-Path $DartOutput "rust_scrcpy_api.dart"
        @"
// Compatibility barrel for split FRB outputs.
//
// FRB now generates APIs under `bridge_generated/rust_scrcpy_api/{model,runtime,service}.dart`.
...
"@ | Set-Content -Path $CompatFile -Encoding UTF8
    }

    Invoke-Step "Flutter freezed/build_runner 代码生成" {
        Set-Location $FlutterRoot
        # 说明：
        # FRB 生成的 rust_scrcpy_api.dart 使用了 freezed 注解，
        # 若不执行 build_runner，将缺少 rust_scrcpy_api.freezed.dart，
    ...
    Invoke-Step "复制 DLL 到 Flutter thirdparty" {
